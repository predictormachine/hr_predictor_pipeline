...
def build_matchup_features(date: str) -> pd.DataFrame:
    ...
    if "batter" not in lu.columns and "batter_id" in lu.columns:
        lu = lu.rename(columns={"batter_id":"batter"})
    if "pitcher" not in lu.columns and "pitcher_id" in lu.columns:
        lu = lu.rename(columns={"pitcher_id":"pitcher"})
    lu["batter"] = lu["batter"].astype(str)
    lu["pitcher"] = lu["pitcher"].astype(str)
    df = lu.merge(bats, on="batter", how="left").merge(pits, on="pitcher", how="left")
    df["recent_hr_rate"]  = df["recent_hr_rate"].fillna(0)
    df["barrel_rate"]     = df["barrel_rate"].fillna(0)
    df["hr_rate_allowed"] = df["hr_rate_allowed"].fillna(0)
    df["composite_score"] = (
